...
const seoContents = [
    {
        key: 'volume_seo_volume-1-la-science-de-la-force',
        value: `Bienvenue dans le Volume 1 de l'Encyclopédie NA Coaching, entièrement consacré à l'art et la science de devenir plus fort. 
...
Explorez nos chapitres détaillés ci-dessous pour maîtriser les concepts clés comme le RPE, le volume effectif, la surcharge progressive, et l'anatomie fonctionnelle appliquée à la musculation.`
    ...
    {
        key: 'volume_seo_volume-2-la-science-de-lendurance',
        value: `Bienvenue dans le Volume 2 de l'Encyclopédie NA Coaching : le guide ultime pour bâtir un cardio inépuisable.
...
Découvrez dans les chapitres ci-dessous les secrets de l'entraînement polarisé, le travail au seuil, et la physiologie de l'effort continu.`
    ...
    {
        key: 'volume_seo_volume-3-la-science-de-la-sante',
        value: `Le Volume 3 de l'Encyclopédie NA Coaching est dédié au pilier fondamental de la performance : la Santé et la Longévité.
...
async function seedVolumeSEO() {
    console.log("Seeding SEO texts for Encyclopedia Volumes into site_content...");

    for (const content of seoContents) {
        ...
        if (existing) {
            console.log(`Updating existing key: ${content.key}`);
            ...
            console.log(`Inserting new key: ${content.key}`);
            const { error } = await supabase
                .from('site_content')
                .insert([{ key: content.key, value: content.value }]);
            if (error) console.error(error);
        }
    }

    console.log("✅ Volume SEO texts successfully seeded.");
}
