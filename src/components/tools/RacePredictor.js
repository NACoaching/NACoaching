        ...
        <div className="bg-white p-8 rounded-2xl border border-zinc-200 shadow-xl max-w-2xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest">Running • Formule de Riegel</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    <label className="block text-xs font-black uppercase text-zinc-700 mb-2">Distance de Référence</label>
                    <select
                        value={refDist}
                ...
                <div>
                    <label className="block text-xs font-black uppercase text-zinc-700 mb-2">Temps Réalisé</label>
            ...
            {predictions ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-xs font-black uppercase text-zinc-600 tracking-widest mb-4">Tes Prédictions Théoriques</h3>
                    <div className="space-y-3">
                        {predictions.map((p) => (
                            <div key={p.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${p.isRef ? 'bg-blue-50 border-blue-200' : 'bg-zinc-50 border-zinc-100'}`}>
                                <div>
                                    <div className="text-xs font-black uppercase text-zinc-700 flex items-center gap-2">
                                ...
                                <div className="text-right">
                                    <div className="text-[10px] font-bold text-zinc-600 uppercase">Allure cible</div>
                ...
                <div className="p-12 border-2 border-dashed border-zinc-100 rounded-2xl flex flex-col items-center justify-center text-center opacity-50">
                    <Clock size={40} className="text-zinc-400 mb-4" />
                    <p className="text-sm font-bold text-zinc-600 uppercase tracking-widest">Entrez votre temps de référence</p>
                </div>
            )}
