    ...
    return (
        <div className="bg-white p-8 rounded-2xl border border-zinc-200 shadow-xl max-w-2xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 className="text-2xl font-black uppercase text-zinc-900">Prédicteur de Performance</h2>
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Running • Formule de Riegel</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    <label className="block text-xs font-black uppercase text-zinc-500 mb-2">Distance de Référence</label>
                    <select
                        value={refDist}
                ...
                <div>
                    <label className="block text-xs font-black uppercase text-zinc-500 mb-2">Temps Réalisé</label>
                    <div className="flex items-center gap-2">
                        <div className="flex-1 min-w-0 text-center">
                            ...
                            <span className="text-[10px] uppercase font-black text-zinc-400">h</span>
                        </div>
                        <div className="flex-1 min-w-0 text-center">
                            ...
                            <span className="text-[10px] uppercase font-black text-zinc-400">m</span>
                        </div>
                        <div className="flex-1 min-w-0 text-center">
                            ...
                            <span className="text-[10px] uppercase font-black text-zinc-400">s</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {predictions ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <h3 className="text-xs font-black uppercase text-zinc-400 tracking-widest mb-4">Tes Prédictions Théoriques</h3>
                    <div className="space-y-3">
                        {predictions.map((p) => (
                            <div key={p.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${p.isRef ? 'bg-blue-50 border-blue-200' : 'bg-zinc-50 border-zinc-100'}`}>
                                <div>
                                    <div className="text-xs font-black uppercase text-zinc-500 flex items-center gap-2">
                                ...
                                <div className="text-right">
                                    <div className="text-[10px] font-bold text-zinc-400 uppercase">Allure cible</div>
                                    <div className="text-md font-black text-zinc-900">{p.pace} min/km</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="p-12 border-2 border-dashed border-zinc-100 rounded-2xl flex flex-col items-center justify-center text-center opacity-50">
                    ...
                    <p className="text-sm font-bold text-zinc-400 uppercase tracking-widest">Entrez votre temps de référence</p>
                </div>
            )}
